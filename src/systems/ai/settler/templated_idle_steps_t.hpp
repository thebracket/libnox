...
namespace work {
	...
	struct templated_idle_steps_t {

		template <typename REGISTER, typename DISPATCH>
		void do_idle(const REGISTER &&registration, const DISPATCH &dispatch, const char * new_status) 
		{
			using namespace bengine;

			// Handle jobs board registration
			if (first_run) {
				jobs_board::register_idle_offer<WORK_TAG>(registration);
				first_run = false;
			}
	...
	inline void get_tool(bengine::entity_t &e, templated_idle_steps_t<WORK_TAG> &work, WORK_TAG &job, 
	...
	template <typename WORK_TAG>
	inline bool skill_check_or_damage(bengine::entity_t &e, const char * skill_name, const int &difficulty, templated_idle_steps_t<WORK_TAG> &work, const int damage, const char * damage_message) {
		auto stats = e.component<game_stats_t>();
		if (!stats) {
			work.cancel_work_tag(e);
			return false;
